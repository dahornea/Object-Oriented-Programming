...
Vector* makeCopy(Vector* v){
    ...
    int i;
    for(i = 0; i < v->size; i++){
        Offer* o = createOffer(v->data[i]->type, v->data[i]->address, v->data[i]->surface, v->data[i]->price);
        if (o == NULL){
            destroyVector(newVector);
            return NULL;
        }
        addToVector(newVector, o);
...
void testAddToVector(){
    ...
    Offer* o = createOffer("apartment", "Molidului", 40, 20000);
    addToVector(v, o);
